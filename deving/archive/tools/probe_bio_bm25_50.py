#!/usr/bin/env python3
"""BIO r3 trial24 with three Takoyaki-inspired 50-shot BM25 variants."""
import argparse
...
from jev.retrieval_variants import Retriever
...
def main():
    ...
    parser.add_argument('--variant', choices=['bigram','trigram','word'], required=True)
    args=parser.parse_args();split='trial';variant=args.variant
    out=OUT/variant;calls=out/'cache/calls';calls.mkdir(parents=True,exist_ok=True)
    ...
    retrievers={c:Retriever(load_jsonl(DATA/'track_a/subtask_2'/c[:3]/f'{c}_train_alltasks.jsonl'),c,banned,variant) for c in corpora}
    config={'split':split,'model':DEFAULT_MODEL,'shots':50,'variant':variant,'jieba_version':'0.42.1','threshold':.65,
            'retrieval':'BM25 k1=1.5 b=0.75; lowercase; char ngrams remove whitespace; Chinese word jieba precise HMM=False; other word whitespace; short text fallback; ID tie-break',
            'dataset':json.loads((ROOT/'data-version.json').read_text())['upstream_commit'],
            'fingerprint':digest({p:(ROOT/p).read_text() for p in ['tools/probe_bio_bm25_50.py','jev/extraction.py','jev/retrieval_variants.py','jev/fewshot.py','tools/diagnose_span_choice.py']})[:12],
    ...
    class Cached:
        def ask(self,state,questions):
            ...
            with key_lock:
                ...
                return result
    def run(job):
        c,r=job;chosen=retrievers[c].select(r['Text'],50);demos=[demonstration(x) for x in chosen]
        ...
        try:
            if path.exists():pred=json.loads(path.read_text())
                ...
                save(path,pred)
            print(f'{variant} {c} {r["ID"]} done; new input={spent}',flush=True)
